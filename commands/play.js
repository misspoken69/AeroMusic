...
exports.run = async (client, message, args) => {
    ...
    if(!channel) return message.channel.send('Hmm, you are not in a voice channel to play music!')

    if (!channel.permissionsFor(message.client.user).has("CONNECT")) return message.channel.send('I don\'t have permission to join that voice channel.')
    if (!channel.permissionsFor(message.client.user).has("SPEAK"))return message.channel.send('I don\'t have permission to speak or play music in the voice channel!')
    ...
    var timeString = date.toISOString().substr(11, 8);
      ...
      if (server) {
        ...
        .setFooter('❤️ Made By Misspoken#1122.')
